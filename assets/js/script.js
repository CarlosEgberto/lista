...
function novoProduto() {
  ...
  if (nomeProduto === '') {
    alert('Digite o nome do produto'); // Exibe um alerta se o campo estiver vazio.
  ...
  if (minhaLista.some(item => item.produtos === nomeProduto)) {
    alert('Produto já cadastrado'); // Exibe um alerta se o produto já existir.
...
function solicitarPreco(index) {
  ...
  if (isNaN(preco) || preco < 0) {
    alert("Preço inválido. Digite um número maior ou igual a zero."); // Exibe um alerta se o preço for inválido.
